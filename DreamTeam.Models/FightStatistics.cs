...
namespace DreamTeam.Models
{
    public class FightStatistics : IFightStatisticsExt
    {
        ...
        {
            public AggregatedData(string skill, float perSecond, float total, float percent)
            {
                Skill = skill;
                PerSecond = perSecond;
                Total = total;
        ...
        internal static IReadOnlyCollection<AggregatedData> AggregateDps(IFight fight, IFighter fighter)
        {
            ...
            foreach (var changeInfo in fight.Statistics.Changes)
                if (changeInfo.Source == fighter)
                {
                    result.Add(new AggregatedData(fighter.ToString(), 1.23f, 3.21f, 45.56f));
                }
        ...
        internal static IReadOnlyCollection<AggregatedData> AggregateHps(IFight fight, IFighter fighter)
        {
            ...
            foreach (var pair in dict)
            {
                var perSecond = totalSeconds > 0
                    ...
                    : 0;
                result.Add(new AggregatedData(pair.Key.Name, perSecond, pair.Value, 100 * pair.Value / total));
            }
